...
struct hisi_chan_entry {
	...
	struct hisi_chan_entry *next;
};

struct worker_thread {
	struct hisi_chan_entry	*ctx;
	struct worker_thread	*next;
	unsigned		core;
};

struct hisi_task {
	struct hisi_chan_entry *hisi_chan_entry;
	void *src;
	void *dst;
};
...
static int g_hisi_chan_num = 0;

static void submit_single_xfer(struct hisi_chan_entry *hisi_chan_entry, struct hisi_task *hisi_task,
			       void *dst, void *src);
...
hisi_done(void *cb_arg)
{
	...
	struct hisi_chan_entry *hisi_chan_entry = hisi_task->hisi_chan_entry;

	if (g_user_config.verify && memcmp(hisi_task->src, hisi_task->dst, g_user_config.xfer_size_bytes)) {
		hisi_chan_entry->xfer_failed++;
	} else {
		hisi_chan_entry->xfer_completed++;
	}

	hisi_chan_entry->current_queue_depth--;

	if (hisi_chan_entry->is_draining) {
		spdk_mempool_put(hisi_chan_entry->data_pool, hisi_task->src);
		spdk_mempool_put(hisi_chan_entry->data_pool, hisi_task->dst);
		spdk_mempool_put(hisi_chan_entry->task_pool, hisi_task);
	} else {
		submit_single_xfer(hisi_chan_entry, hisi_task, hisi_task->dst, hisi_task->src);
	}
}
...
unregister_workers(void)
{
	...
	struct hisi_chan_entry *entry, *entry1;

	/* Free hisi_chan_entry and worker thread */
	while (worker) {
...
attach_cb(void *cb_ctx, struct spdk_pci_device *pci_dev, struct spdk_hisi_chan *hisi)
{
	...
	if (g_hisi_chan_num >= g_user_config.hisi_chan_num) {
		return;
	}
...
parse_args(int argc, char **argv)
{
	...
	while ((op = getopt(argc, argv, "c:hn:o:q:t:v")) != -1) {
		...
		case 'o':
			...
			break;
		case 'n':
			g_user_config.hisi_chan_num = spdk_strtol(optarg, 10);
			break;
		case 'q':
...
static void
drain_io(struct hisi_chan_entry *hisi_chan_entry)
{
	while (hisi_chan_entry->current_queue_depth > 0) {
		spdk_hisi_process_events(hisi_chan_entry->chan);
	}
}

static void
submit_single_xfer(struct hisi_chan_entry *hisi_chan_entry, struct hisi_task *hisi_task, void *dst,
		   void *src)
{
	hisi_task->hisi_chan_entry = hisi_chan_entry;
	hisi_task->src = src;
	hisi_task->dst = dst;

	spdk_hisi_submit_copy(hisi_chan_entry->chan, hisi_task, hisi_done, dst, src,
			     g_user_config.xfer_size_bytes);
	hisi_chan_entry->current_queue_depth++;
}

static int
submit_xfers(struct hisi_chan_entry *hisi_chan_entry, uint64_t queue_depth)
{
	while (queue_depth-- > 0) {
		void *src = NULL, *dst = NULL;
		struct hisi_task *hisi_task = NULL;

		src = spdk_mempool_get(hisi_chan_entry->data_pool);
		dst = spdk_mempool_get(hisi_chan_entry->data_pool);
		hisi_task = spdk_mempool_get(hisi_chan_entry->task_pool);
		if (!hisi_task) {
			fprintf(stderr, "Unable to get hisi_task\n");
			return 1;
		}

		submit_single_xfer(hisi_chan_entry, hisi_task, dst, src);
	}
	return 0;
}
...
dump_result(void)
{
	...
	printf("-----------------------------------------------------------\n");
	while (worker != NULL) {
		struct hisi_chan_entry *t = worker->ctx;
		while (t) {
			...
			if (xfer_per_sec) {
				printf("%10d%10d%12" PRIu64 "/s%8" PRIu64 " MiB/s%11" PRIu64 "\n",
				       t->hisi_chan_id, worker->core, xfer_per_sec,
...
{
	struct spdk_hisi_chan *chan = get_next_chan();
	struct worker_thread	*worker = g_workers;
	struct hisi_chan_entry	*t;
	char buf_pool_name[30], task_pool_name[30];
	int i = 0;

	while (chan != NULL) {
		t = calloc(1, sizeof(struct hisi_chan_entry));
		if (!t) {
			return 1;
		}
		...
		printf("Associating hisi_channel %d with core %d\n", i, worker->core);
		t->chan = chan;
		t->next = worker->ctx;
		worker->ctx = t;
...
main(int argc, char **argv)
{
	...
	if (g_hisi_chan_num == 0) {
		printf("No channels found\n");
		rc = 1;
		goto cleanup;
	}

	if (g_user_config.hisi_chan_num > g_hisi_chan_num) {
		printf("%d channels are requested, but only %d are found,"
		       "so only test %d channels\n", g_user_config.hisi_chan_num,
		       g_hisi_chan_num, g_hisi_chan_num);
		g_user_config.hisi_chan_num = g_hisi_chan_num;
	}
